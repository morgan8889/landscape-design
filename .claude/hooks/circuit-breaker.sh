...
# Prune entries older than 2 hours (only when log exceeds 200 lines)
...
if [ -f "$LOG_FILE" ]; then
  ...
  MATCH_COUNT=$(tail -n "$THRESHOLD" "$LOG_FILE" | cut -d'|' -f2- | grep -cxF "$NORMALIZED" || true)

  if [ "$MATCH_COUNT" -ge "$THRESHOLD" ]; then
    # Output block decision
    cat <<BLOCK
CIRCUIT BREAKER TRIPPED: The command below has been attempted $MATCH_COUNT times consecutively:
